...
- Progress is **the bytes already handed to the file, which a cancel will not throw away**: the number never goes backwards (it used to be reported from bytes received, i.e. in-memory data that is discarded on pause — the UI would show 40MB and drop to the 1.4MB actually on disk). **Speed** is measured separately from bytes received (3-second window), so tightening up the progress rule does not bring back the "speed keeps reading 0" problem
