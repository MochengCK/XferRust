...
- The output name is derived from the playlist URL you provided: the `.m3u8` extension is dropped, and generic last segments such as `index` / `playlist` / `master` fall back to the parent directory name. The `out` option still takes precedence as before.
