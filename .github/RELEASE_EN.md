...
- **A total size is available from the very first second**: the master playlist's `BANDWIDTH x duration` gives an initial estimate (+/-10%), corrected by the measured extrapolation once the first segment lands. Previously the total only appeared after the first segment was written, so on a rate-limited CDN the UI showed neither a size nor a percentage for that whole stretch
