...
- 外部传入默认参数：任意 `--key=value` 一律宽容接受——已实现的键启动即生效；引擎暂未实现的键不再「告警后丢弃」，而是同样存入全局选项存储（`engine.getOptions` 可读回、随会话持久化），语义由调用方决定。宿主应用可以把完整配置透传给引擎命令行，引擎升级后无需改动启动参数即自动承接新选项；仅裸位置参数（非 `--` 开头）视为无效参数告警忽略
...
- 单任务限速：`task.changeOption` 支持 `max-download-limit` / `max-upload-limit`（aria2 语义，`1M`/`500K` 单位直通），实际生效值 = 单任务优先覆盖全局（单任务已设置时优先生效，可高于也可低于全局；未设置 0 = 跟随全局），运行时热生效并随会话持久化；HTTP 每个任务持有独立限速器承载合成值（单连接与 split 多连接共享，全局或任务级变更即时重新同步），BT 由各 TorrentEngine 按合成值下发；`task.getOption` 输出当前任务限速值。全局限速自此对 HTTP/HTTPS 与 BT 全量生效
...
- macOS 引擎内核产物按架构拆分：TUI 仍为双架构通用二进制（`xfer-tui-darwin-universal.tar.gz`），引擎内核不再合并，改为 `xferrust-darwin-aarch64.tar.gz` 与 `xferrust-darwin-x86_64.tar.gz` 两个独立包——嵌入集成的应用端可按目标架构直取对应产物，无需再从通用二进制做 thin 提取
