...
除上方核心参数外，任何 `--key=value` 形式的选项都会被接受为**外部传入的
默认参数**：引擎已实现的键启动即生效，暂未实现的键存入全局选项存储
（`engine.getOptions` 可读回），不报错、不告警——宿主应用可以放心把完整
配置透传给引擎。
...
其余任意 `--key=value` 均作为外部默认参数接受（见「命令行使用指南 → 守护进程」）。
