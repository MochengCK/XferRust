...
impl FileSink {
    ...
    /// 只把 `BufWriter` 的缓冲交给内核，**不做 fsync**。
    ///
    /// 用途：长任务的**中途**水位保存。`sync_all` 会等待该文件的全部脏页写回
    /// 磁盘 —— 对一个已经写入数 GB 的产物，一次 `fsync` 要等几百 MB 脏页落盘
    /// （实测秒级），而中途保存是**每秒一次**的（控制文件节流），于是整个
    /// 拼接阶段都在等磁盘，合并速度被它拖住。
    ///
    /// 安全性：`write(2)` 返回即数据已在内核页缓存里，**进程崩溃/被 kill 都
    /// 不会丢**，只有断电/内核崩溃才可能丢；而终局（完成、暂停收尾）仍走
    /// [`Self::flush`]（含 `sync_all`）兜住断电场景。中途水位与控制文件因此
    /// 与"已交给内核的字节"严格对齐，不会出现"控制文件领先磁盘"。
